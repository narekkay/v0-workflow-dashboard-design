      ...
      {isLoading && (
        <div className="absolute inset-0 bg-white z-50 flex items-center justify-center transition-opacity duration-300">
          <div className="text-center">
            <svg className="animate-spin h-12 w-12 text-slate-900 mx-auto mb-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
            <p className="text-lg font-medium text-slate-900">Connexion...</p>
            <p className="text-sm text-slate-600 mt-2">Chargement de votre espace</p>
          </div>
        </div>
      )}
            ...
            <form onSubmit={handleLogin} className="space-y-5">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className="h-12 border-2 border-slate-200 focus:border-slate-400 transition-colors"
                />
              </div>
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <Label htmlFor="password" className="text-slate-700 font-medium">Mot de passe</Label>
                  <button type="button" className="text-sm text-slate-600 hover:text-slate-900 hover:underline">
                    Mot de passe oublié ?
                  </button>
                </div>
                <Input
              ...
              {error && (
                <div className="text-sm text-red-600 text-center bg-red-50 py-3 px-4 rounded-lg border-2 border-red-200">
                  {error}
                </div>
              )}
              <Button type="submit" className="w-full h-12 text-base font-semibold bg-[#171717] hover:bg-[#171717]/90 text-white transition-colors shadow-md" disabled={isLoading}>
                {isLoading ? (
