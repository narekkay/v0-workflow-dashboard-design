...
interface FormIFUViewProps {
  clientId: string
  clientName: string
}

// Using external Vercel Blob URL - local /public files don't work reliably in v0 runtime
const PDF_URL = "https://blobs.vusercontent.net/blob/formulaire%20IFU-OyburGxBbNyt9jx6jkBjkjBiR8WozS.pdf"

export function FormIFUView({ clientId, clientName }: FormIFUViewProps) {
  const handleDownload = () => {
    const link = document.createElement('a')
    link.href = PDF_URL
    link.download = 'formulaire_IFU.pdf'
    link.click()
  }
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="sticky top-0 z-10 bg-background flex items-center justify-between pb-4 border-b pt-4 px-6">
        <h1 className="text-3xl font-bold">IFU</h1>
